...
【提取原则】
1. 只提取用户（role: user）说的关于其自身的个人信息，忽略由于寒暄、提问天气、咨询知识等产生的信息。
2. 重要性(importance)范围 0.1~1.0：职业方向/目标计划 >= 0.8，个人背景 0.6，情感状态 0.5。
3. 如果用户只是在问问题（例如“某地天气怎么样”、“某某名词是什么意思”），系统绝对没有需要记忆的偏好信息，你必须直接返回 []！绝对不要在此处回答用户的问题。
